...
- (id)initWithAppId:(NSString *)app_id reDirectURL:(NSString *)kRedirectURL secretKey:(NSString *) key{
  ...
  if (self) {
	...
	_kRedirectURL = [kRedirectURL copy];
	  [_secretKey release];
	  _secretKey = [key copy];
      self.accessToken = [[NSUserDefaults standardUserDefaults] valueForKey:@"access_Token"] ;
  }
  return self;
}
...
-(void)createUserSessionInfo{
	...
	if (self.expirationDate) {
		[defaults setObject:self.expirationDate forKey:@"kaixin_expiration_Date"];
	}
...
- (void)authorizeWithKXAppAuth
{
    NSMutableDictionary* params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   ...
                                   @"iPhone",     @"display",
                                   @"basic%20upload_photo%20user_photo%20create_album%20create_records",   @"scope",//要多权限时，以%20串联各个权限标记，例如：“basic%20friends_records”
                                   @"1",@"oauth_client",
                                   nil];
