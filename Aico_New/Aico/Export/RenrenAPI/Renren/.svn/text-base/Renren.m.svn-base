...
-(BOOL)isSessionValid{
	...
	if (nil != defaults){
		self.accessToken = [defaults objectForKey:@"access_Token1"];
		self.expirationDate = [defaults objectForKey:@"expiration_Date1"];
...
-(void)saveUserSessionInfo{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];	
    if (self.accessToken) {
        [defaults setObject:self.accessToken forKey:@"access_Token1"];
    }
	if (self.expirationDate) {
		[defaults setObject:self.expirationDate forKey:@"expiration_Date1"];
	}	
    if (self.sessionKey) {
...
- (void)requestWithParam:(RORequestParam *)param andDelegate:(id <RORequestDelegate>)delegate {
    ...
    if (![self isSessionValid]) {
        ...
        if ([self.renrenDelegate respondsToSelector:@selector(renrenAuthorizeExpired)])
        {
            [self delUserSessionInfo];
            [self.renrenDelegate renrenAuthorizeExpired];
        }
        return;
    }
...
- (void)authDialog:(id)dialog withOperateType:(RODialogOperateType )operateType{
    ...
    switch (operateType) {
        case RODialogOperateSuccess:
            ...
            if (_renrenDelegate == nil)
            {
                NSLog(@"renren delegate 是空的");
            }
            else
            {
            //用户信息保存到本地
 ...
 * 用户oauth2登录请求认证授权
 */
- (void)authorizationWithPermisson2:(NSArray *)permissions andDelegate:(id<RenrenDelegate>)delegate andOnView:(UIView *)onView 
{
    NSHTTPCookieStorage* cookies = [NSHTTPCookieStorage sharedHTTPCookieStorage];
	NSArray* graphCookies = [cookies cookiesForURL:
                             [NSURL URLWithString:@"http://graph.renren.com"]];
    ...
    if (![self isSessionValid])
    {
        ...
        //即kAPI_Key
        [parameters setValue:_appKey forKey:@"client_id"];
        //kRRSuccessURL @"http://widget.renren.com/callback.html"
        [parameters setValue:kRRSuccessURL forKey:@"redirect_uri"];
        [parameters setValue:@"token" forKey:@"response_type"];
        [parameters setValue:@"touch" forKey:@"display"];
        if (nil != self.permissions) 
        ...
        viewController.params = parameters;
        viewController.delegate = self;
        //kAuthBaseURL @"http://graph.renren.com/oauth/authorize"
        viewController.serverURL = kAuthBaseURL;
        [viewController show];
 ...
 * 用户oauth2登录请求认证授权
 */
- (void)authorizationWithPermisson3:(NSArray *)permissions andDelegate:(id<RenrenDelegate>)delegate andOnView:(UIView *)onView 
{
    NSHTTPCookieStorage* cookies = [NSHTTPCookieStorage sharedHTTPCookieStorage];
	NSArray* graphCookies = [cookies cookiesForURL:
                             [NSURL URLWithString:@"http://graph.renren.com"]];
...
- (void)dialog:(NSString *)action andParams:(NSMutableDictionary *)params andDelegate:(id<RenrenDelegate>)delegate{
	NSString *dialogURL = [kRRDialogBaseURL stringByAppendingString:action];
	...
	if ([self isSessionValid]) {
        [params setValue:[self.accessToken stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] forKey:@"access_token1"];
    }
...
- (void)request:(RORequest *)request didLoad:(id)result{
	...
	if([request.requestParamObject isKindOfClass:[ROPasswordFlowRequestParam class]])
	{
		NSString *token = [request.responseObject.rootObject objectForKey:@"access_token1"];
		NSString *date = [request.responseObject.rootObject objectForKey:@"expires_in"];
			 
		self.accessToken = [request.responseObject.rootObject objectForKey:@"access_token1"];;
...
//不需要[dialogView show:YES];
- (void)publishPhotoSimplyWithImage2:(UIImage *)image caption:(NSString *)caption
{
