...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
            ...
            //----modify by jiangliyin 2012-1-31 for memory leaks start--------------//
            CFNumberRef cfsize = CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount);
            [self setTotalBytesSent:[NSMakeCollectable((NSNumber *)cfsize) unsignedLongLongValue]];
            CFRelease(cfsize);
            
//			[self setTotalBytesSent:[NSMakeCollectable((NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount)) unsignedLongLongValue]];
            //----modify by jiangliyin 2012-1-31 for memory leaks end--------------//
...
{
    CFErrorRef underError = CFReadStreamCopyError((CFReadStreamRef)[self readStream]);
	NSError *underlyingError = NSMakeCollectable((NSError *)underError );
	...
	if (![self error]) { // We may already have handled this error
		...
		if (([[underlyingError domain] isEqualToString:NSPOSIXErrorDomain] && ([underlyingError code] == ENOTCONN || [underlyingError code] == EPIPE)) 
			...
			if ([self retryUsingNewConnection]) {
                //----add by jiangliyin 2012-1-31 for memory leaks start--------------//
                CFRelease(underError);
                //----add by jiangliyin 2012-1-31 for memory leaks end--------------//
				return;
			}
		}
