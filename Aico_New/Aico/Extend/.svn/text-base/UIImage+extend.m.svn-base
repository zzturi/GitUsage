 ...
 * @note
 */
+ (void)duoTone:(ImageInfo *)imageInfo withColor:(NSArray *)colorArray withLevel:(float)value
{    
    unsigned char *imgPixel = imageInfo.data;
	GLuint imgWidth = imageInfo.width;
	GLuint imgHeight = imageInfo.height;
    ...
    GLuint imgBytesPerRow = imageInfo.bytesPerRow;
    int offset = imgBytesPerRow - imgWidth * imgBpp;
    NSNumber* r1 = [colorArray objectAtIndex:0];
    NSNumber* g1 = [colorArray objectAtIndex:1];
    NSNumber* b1 = [colorArray objectAtIndex:2];
    NSNumber* r2 = [colorArray objectAtIndex:3];
    NSNumber* g2 = [colorArray objectAtIndex:4];
    NSNumber* b2 = [colorArray objectAtIndex:5];
	...
	for(GLuint y = 0;y< imgHeight;y++) 
    {
		for (GLuint x = 0; x<imgWidth; x++) 
        {
            ...
            {
                red = (r2.intValue - r1.intValue) / (2 * value ) * (gray - 128 + value) + r1.intValue;                
                green = (g2.intValue - g1.intValue) / (2 * value ) * (gray - 128 + value) + g1.intValue;
                blue = (b2.intValue - b1.intValue) / (2 * value ) * (gray - 128 + value) + b1.intValue;
            }
            else
            {
                red = r2.intValue;                
...
//    [EffectMethods gaussianBlurWithImageInfo:clone andRadius:5];
    [ColorEffectsCommon fastBlurImage:clone radius:8];
    // 对拷贝图适当调整亮度
        ...
        withOpacity:(float)opacity
{
    ImageInfo *clone = [imageInfo clone];
    // 边缘检测
    [EffectMethods neonWithInImage:imageInfo];  
//    [EffectMethods piexlSingleWithInImageInfo:imageInfo];
...
+ (void)neonAtImage:(ImageInfo *)imageInfo 
        ...
        withOpacity:(float)opacity
{
    NSLog(@"%f,%f,%f,%f",hue,saturation,bright,opacity);
    ImageInfo *clone = [imageInfo clone];
    // 边缘检测
    [EffectMethods neonWithInImage:imageInfo]; 
//    [EffectMethods piexlSingleWithInImageInfo:imageInfo];
    [EffectMethods addColourToImageInfo:imageInfo hue:hue saturation:saturation];
